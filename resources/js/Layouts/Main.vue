<template>
    <v-app id="inspire" style="background: #eee">
        ...
        <!--App Bar-->
        <v-app-bar id="core-toolbar" style="background: #eee" app flat>
...
export default {
    ...
    computed: {
        ...mapGetters(["getSnackbar", "getApp", "serverUrl"]),
    },
    methods: {
        logout() {
            axios.post(this.serverUrl + "/logout", {}).then((response) => {
                this.$router.go(0);
            });
        },
    },
    ...
    mounted() {
        ...
        return new Promise((resolve, reject) => {
            this.$store.dispatch("getMe").then((response) => {});
        });
    },
};
</script>
