    ...
    <div class="container">
        <!--Table View Started-->
        <v-btn color="blue darken-1" text @click="dialog = true">Add user2</v-btn>
    ...
    data() {
        ...
        loading: true,
        headers: [
            { text: "昵称", align: "left", sortable: false, value: "name" },
        ...
        usernameRules: [
            ...
            (v) =>
            (v && v.length >= 6) || "Profilename can't be less than 6 characters",
        ],
        editedIndex: -1,
        editedItem: {
            name: "",
    ...
    methods: {
        ...
        fetch(){
            axios.get('/reglist',{params: this.options})
                    .then(response =>{
                        this.data = response.data.data;
        ...
        if (this.$refs.form.validate()) {
            return new Promise((resolve, reject) => {
            this.$store
                .dispatch("saveUser", this.editedItem)
                .then((response) => {
        ...
        this.$root
            .$confirm("Delete " + item.name, "Are you sure?", { color: "red" })
            .then((confirm) => {
            if (confirm) {
                //if true, call delete end point
                this.$store.dispatch("deleteUser", item.id).then((response) => {
                this.$store.dispatch(
                    "showSuccessSnackbar",
